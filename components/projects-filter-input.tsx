...
import { useProjectsFilters } from "@/app/projects/use-projects-filters";
import { Button } from "@/components/ui/button";
import { CloseIcon } from "@/components/icons";

export default function ProjectsFilterInput({
  className,
}: {
  className?: string;
}) {
  const { tokens, addRaw, removeAt, apply, reset } = useProjectsFilters();
  const [input, setInput] = useState("");
  const inputRef = useRef<HTMLInputElement | null>(null);
  useEffect(() => {
    const h = setTimeout(() => apply(), 150);
    ...
    >
      <div className="flex items-center gap-2 flex-wrap max-h-20 overflow-y-auto pr-1">
        ...
        <input
          ...
          placeholder="Type filter then Enter (id:5 person:12 status:inactive)"
        />
        <Button
          type="button"
          variant="outline"
